...
namespace XnbExtractor.Compression
{
    ...
    class LzxDecoder
	{
		...
		public int Decompress(Stream inData, int inLen, Stream outData, int outLen)
		{
			...
			while(togo > 0)
			{
				...
				while((this_run = (int)m_state.block_remaining) > 0 && togo > 0)
				{
                    ...
                    if ((window_posn + this_run) > window_size)
                        throw new InvalidDataException("Window overflow");
            ...
            if (togo != 0)
                throw new InvalidDataException($"togo = {togo}");
            int start_window_pos = (int)window_posn;
			if(start_window_pos == 0) start_window_pos = (int)window_size;
			start_window_pos -= outLen;
            Console.WriteLine($"Writing {outLen} bytes");
            Console.WriteLine($"start_window_pos = {start_window_pos}");
            Console.WriteLine($"window_posn = {window_posn}");
