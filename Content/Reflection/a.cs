...
namespace XnbExtractor.Content.Reflection
{
    using isdf.Runtime;
    ...
    public class ContentManager : IDisposable
    {
        ...
        public static T LoadXnb<T>(string filename)
        {
            ...
            using var xnb = new XnbFile(filename);

            var decompressedData = xnb.Decompress();

            XnbContent content = XnbContentReader.ParseContent(decompressedData);
            using var reader = new XnbContentReader( new MemoryStream(decompressedData), content, xnb.Header.Platform);
            return (T)XnbTypeReaderFactory.ReadByType(
                content.Readers[content.PrimaryReaderIndex - 1],
                reader);
        }
        public void Test(string contentRoot)
        {
            var palettes = new List<TEdgePal>();

            for (int i = 0; ; i++)
            {
                string path = Path.Combine(contentRoot, $"data\\pal{i}.xnb");

                if (!File.Exists(path))
                    break;

                palettes.Add(LoadXnb<TEdgePal>(path));
            }

            var paletteManager = new TEdgePalManager(palettes.ToArray());

            var kao = LoadXnb<TBmp256>(
                Path.Combine(contentRoot, "bmp\\char\\kao.xnb"));

            Console.WriteLine($"{kao.Width}x{kao.Height}");
        }

        private object LoadTBmp256(string v)
        {
            throw new NotImplementedException();
        }

        private TEdgePal LoadTEdgePal(string path)
        {
            throw new NotImplementedException();
        }
        ...
        //
        // Summary:
        //     Releases the unmanaged resources used by the ContentManager and optionally releases
        ...
        //     Delegate function for handling the disposition of assets. If recordDisposableObject
        //     is null, the default lifespan tracking and management is used, so unloading or
        //     disposing of the content manager frees everything that has been loaded through
        //     it. If recordDisposableObject specifies a valid delegate, that delegate is used
        //     instead of the default lifespan tracking and is called every time the loader
        //     encounters a type that implements IDisposable. You must use your own code to
        //     unload assets loaded in this fashion, since ContentManager's Unload method will
...
/// <summary>
/// A ContentReader that wraps our existing XnbContentReader.
/// </summary>
public class ReflectionContentReader
    {
        private XnbContentReader _reader;

        public XnbTypeManifest Manifest { get; }

        public ReflectionContentReader(XnbContentReader reader)
        {
            _reader = reader;
        }

        public ReflectionContentReader(XnbContentReader reader, XnbTypeManifest manifest) : this(reader)
        {
            Manifest = manifest;
        }
        ...
        {
            // your type reader dispatch here
            return default;
        }
    ...
    public static class ReflectionReaderFactory
    {
        public static object ReadCustomObject(
            Assembly assembly,
            string readerName,
            XnbContentReader reader,
            XnbTypeManifest manifest)
        {
            Type type = assembly.GetType(readerName);

            if (type == null)
                throw new InvalidOperationException(
                    $"Reader '{readerName}' not found.");

        ContentTypeReader instance =
                (ContentTypeReader)Activator.CreateInstance(type);

            ReflectionContentReader contentReader =
                new ReflectionContentReader(reader, manifest);

            return InvokeReader(instance, contentReader);
        }

        private static object InvokeReader( ContentTypeReader reader, ReflectionContentReader input)
        {
            MethodInfo method =
                reader.GetType().GetMethod(
                    "Read",
                    BindingFlags.Instance |
                    BindingFlags.NonPublic);

            return method.Invoke(reader, new object[] { input, null });
        }
    }
    // Microsoft.Xna.Framework, Version=3.1.0.0, Culture=neutral, PublicKeyToken=6d5c3888ef60e27d
    ...
    internal class FrameworkResources
    {
        ...
        internal static ResourceManager ResourceManager
        {
            get
            {
                ...
                {
                    ResourceManager resourceManager = new ResourceManager("Microsoft.Xna.Framework.FrameworkResources", typeof(FrameworkResources).Assembly);
